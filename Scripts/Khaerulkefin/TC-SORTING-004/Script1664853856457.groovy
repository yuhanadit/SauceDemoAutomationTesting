...
List<WebElement> hargaList = WebUI.findWebElements(findTestObject('Object Repository/Page_Swag Labs/List_Harga'), 10)

List<Double> hargaOri = new ArrayList()
List<Double> hargaSort = new ArrayList()

for (data in hargaList) {
	hargaOri.add(Double.parseDouble(data.getText().replace('$', '')))
	hargaSort.add(Double.parseDouble(data.getText().replace('$', '')))
}

Collections.sort(hargaSort, Collections.reverseOrder())
